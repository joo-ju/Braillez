...
text_start = "안녕하세요. 점자 인식을 시작합니다."
text_done = "점자를 인식하였습니다."
text_fail = "점자 인식에 실패하였습니다. 다시 시도해 주십시오."
tts_start = gTTS(text=text_start, lang='ko')
tts_done = gTTS(text=text_done, lang='ko')
tts_fail = gTTS(text=text_fail, lang='ko')
tts_start.save("hello.mp3")
tts_done.save("done.mp3")
tts_fail.save("fail.mp3")
...
while cap.isOpened():
    ...
    if ret:
        ...
        img = cv.imread("./Images/n.jpg", cv.IMREAD_GRAYSCALE)
        # img = img.astype(np.uint8)
        img = cv.rotate(img, cv.ROTATE_180)
        # cv.imwrite('/home/pi/Raspberry/Images/image.jpg', img)
        # cv.imshow('', img)
        img_2 = cv.imread("./Images/n.jpg")
        ...
        _, contours, _ = cv.findContours(closing1, cv.RETR_TREE, cv.CHAIN_APPROX_NONE)
        center = []
        ...
        rotation1 = np.array(rotation[:, :, 0], dtype=np.uint8)
        _, contours, _ = cv.findContours(rotation1, cv.RETR_TREE, cv.CHAIN_APPROX_NONE)
        for i in range(len(contours)):
            cnt = contours[i]
            area = cv.contourArea(cnt)
            x, y, w, h = cv.boundingRect(cnt)
        ...
        tts_braille = gTTS(text=strA, lang='ko')
        tts_braille.save("braille.mp3")

        os.system("mpg123 braille.mp3")
        break
        if cv.waitKey(1) & 0xFF == 27:  # esc
            break
    else:
        print('no camera!')
        os.system("mpg123 fail.mp3")
        # break
