...
# 점자 점의 크기를 정해놓지 않고 매번 사진마다 계산해서 하는것.
...
# width의 최빈값 상위 3개 구하기
w = Counter(width)
ww = w.most_common(3)

# 최빈값 상위 3개로 평균 구하기 -> 점들의 평균 width
w_ave = int((ww[0][0] + ww[1][0] + ww[2][0]) / 3)
...
print("ww[0][0]", ww[0][0])
# print("width 최빈값 3개 : ", w.most_common(3))
print("width ave = ", w_ave)
print('center', center)
...
for i in range(len(mid_x)-1):
    diff.append(mid_x[i+1] - mid_x[i])


# 중심점과의 차이 중에 가장 작은 값을 골라 반올림 한다.

print("diff : ", diff)
print("diff min : ", min(diff))
print("diff min 1의 자리 반올림: ", round(min(diff), -1))
dis = round(min(diff), -1) / 2
...
# by 김주희_6등분 선중 수직선 좌표과 저장되어 있응 braille을 이용하여 수직선 그리기 & vline 리스트에 선 좌표 넣기 _201024
vvline=[]
for i in range(len(braille)):
    ...
    plt.vlines(braille[i][0], 0, 1944, colors='pink', linewidth=1)
    plt.vlines(braille[i][1], 0, 1944, colors='pink', linewidth=1)
    # plt.vlines(braille[i][1]+20, 0, 1944, colors='pink', linewidth=1)
